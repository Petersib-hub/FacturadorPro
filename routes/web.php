...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    // ➤ Rutas adicionales del módulo recurrente
    Route::post('recurring-invoices/{recurring_invoice}/duplicate', [RecurringInvoiceController::class, 'duplicate'])
        ->name('recurring-invoices.duplicate');

    Route::put('recurring-invoices/{recurring_invoice}/pause', [RecurringInvoiceController::class, 'pause'])
        ->name('recurring-invoices.pause');

    Route::put('recurring-invoices/{recurring_invoice}/resume', [RecurringInvoiceController::class, 'resume'])
        ->name('recurring-invoices.resume');

    Route::post('recurring-invoices/{recurring_invoice}/run-now', [RecurringInvoiceController::class, 'runNow'])
        ->name('recurring-invoices.run-now');

    // Impuestos
    Route::resource('tax-rates', TaxRateController::class);

    /**
     * Registrar "copiar enlace público" y devolver la URL para copiar.
     */
...
// Perfil (Breeze) – solo requiere auth
...
Route::prefix('pub')->name('public.')->group(function () {
    ...
    // Portal del cliente (histórico por token)
