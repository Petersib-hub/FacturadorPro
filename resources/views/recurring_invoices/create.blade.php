@extends('layouts.app')
@section('title','Nueva plantilla recurrente')
...
<form method="post" action="{{ route('recurring-invoices.store') }}">
    ...
    <div class="card card-soft mb-3">
        <div class="card-body">
            <h5 class="mb-3">Nueva plantilla de factura recurrente</h5>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select name="frequency" class="form-select" required>
                        @php
                            $freqs = ['daily'=>'Diaria','weekly'=>'Semanal','monthly'=>'Mensual','quarterly'=>'Trimestral','yearly'=>'Anual'];
                        @endphp
                        @foreach($freqs as $k=>$lbl)
                            <option value="{{ $k }}" @selected(old('frequency','monthly')==$k)>{{ $lbl }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input name="currency" class="form-control" value="{{ old('currency','EUR') }}" required>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="date" name="next_run_date" class="form-control" value="{{ old('next_run_date', now()->addMonth()->toDateString()) }}" required>
                </div>

                <div class="col-12">
                    <label class="form-label">Términos</label>
                    <textarea name="terms" rows="2" class="form-control">{{ old('terms') }}</textarea>
                </div>

                <div class="col-12">
                    <label class="form-label">Notas públicas (en factura)</label>
                    <input name="public_notes" class="form-control" value="{{ old('public_notes') }}">
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table align-middle" id="riItemsTable">
                    ...
                    <tbody>
                        @php
                            $oldItems = old('items', [['product_id'=>null,'description'=>'','quantity'=>1,'unit_price'=>0,'discount'=>0,'tax_rate'=>0]]);
                        @endphp
    ...
    <div class="text-end">
        ...
        <a href="{{ route('recurring-invoices.index') }}" class="btn btn-outline-secondary">Cancelar</a>
    </div>
</form>
@endsection
