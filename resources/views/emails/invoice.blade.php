...
<body>
    <div class="preheader">Factura {{ $invoice->number }} para {{ $client?->name }}. Total: {{ number_format($invoice->total,2,',','.') }} {{ $currency }}</div>
    ...
    <div class="wrap">
        <div class="card">
            ...
            <div class="cnt">
                <h1 class="h1">Factura {{ $invoice->number }}</h1>
                <div class="muted mb-12">
                    @if($client) Para: <strong>{{ $client->name }}</strong>@endif
                    @if($invoice->date) · Fecha: {{ \Illuminate\Support\Carbon::parse($invoice->date)->format('d/m/Y') }} @endif
                    @if($invoice->due_date) · Vence: {{ \Illuminate\Support\Carbon::parse($invoice->due_date)->format('d/m/Y') }} @endif
                </div>

                <table class="table" role="presentation" cellspacing="0" cellpadding="0">
                    <tr>
                        <td class="label">Subtotal</td>
                        <td class="value" style="text-align:right">{{ number_format($invoice->subtotal,2,',','.') }} {{ $currency }}</td>
                    </tr>
                    <tr>
                        <td class="label">Impuestos</td>
                        <td class="value" style="text-align:right">{{ number_format($invoice->tax_total,2,',','.') }} {{ $currency }}</td>
                    </tr>
                    <tr>
                        <td class="label total">TOTAL</td>
                        <td class="value total" style="text-align:right">{{ number_format($invoice->total,2,',','.') }} {{ $currency }}</td>
                    </tr>
                    <tr>
                        <td class="label">Pagado</td>
                        <td class="value" style="text-align:right">{{ number_format($invoice->amount_paid ?? 0,2,',','.') }} {{ $currency }}</td>
                    </tr>
                    <tr>
                        <td class="label">Pendiente</td>
                        <td class="value" style="text-align:right">{{ number_format(($invoice->total ?? 0) - ($invoice->amount_paid ?? 0),2,',','.') }} {{ $currency }}</td>
                    </tr>
                </table>
                ...
                @if($billing)
                <div class="mt-12">
                    <div class="mb-8"><strong>Notas de pago</strong></div>
                    <div class="mb-8">{!! nl2br(e($billing)) !!}</div>
                </div>
                @endif

                {{-- Si usas enlace público, pasa $publicUrl desde el mailable y descomenta: --}}
                {{-- <p class="mt-12"><a class="kb" href="{{ $publicUrl }}" target="_blank" rel="noopener">Ver factura en línea</a></p> --}}
            </div>
