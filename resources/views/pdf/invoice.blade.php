    ...
    <section class="grid mb-3">
        <div class="col col-6">
            ...
            <div class="box">
                ...
                @if($invoice->client->tax_id)<div class="small muted">NIF/CIF: {{ $invoice->client->tax_id }}</div>@endif
                @if($invoice->client->email)<div class="small muted">Email: {{ $invoice->client->email }}</div>@endif
                @if($invoice->client->address)<div class="small muted">{{ $invoice->client->address }}</div>@endif
                @if($invoice->client->city || $invoice->client->zip)
                <div class="small muted">{{ $invoice->client->zip }} {{ $invoice->client->city }}</div>
                @endif
                @if($invoice->client->country)
                <div class="small muted">{{ $invoice->client->country }}</div>
                @endif
            </div>
        </div>
        <div class="col col-6">
            <table class="w-100">
                <tr>
                    <td class="muted small">Fecha</td>
                    <td class="text-right">{{ optional($invoice->date)->format('d/m/Y') }}</td>
                </tr>
                <tr>
                    <td class="muted small">Vence</td>
                    <td class="text-right">{{ optional($invoice->due_date)->format('d/m/Y') ?: '—' }}</td>
                </tr>
                <tr>
                    <td class="muted small">Moneda</td>
