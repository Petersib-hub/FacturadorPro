...
<body>
@php
    $s = \App\Models\UserSetting::firstWhere('user_id', $invoice->user_id);
...
<section class="grid mb-3">
    <div class="col col-6">
        <div class="mb-1" style="font-weight:700;">Facturar a</div>
        <div class="box">
            <div>{{ $c->name }}</div>
    ...
    <div class="col col-6">
        <table class="w-100">
            <tr><td class="muted small">Fecha</td><td class="text-right">{{ optional($invoice->date)->format('d/m/Y') }}</td></tr>
            <tr><td class="muted small">Vence</td><td class="text-right">{{ optional($invoice->due_date)->format('d/m/Y') ?: '—' }}</td></tr>
            <tr><td class="muted small">Moneda</td><td class="text-right">{{ $invoice->currency }}</td></tr>
            @if(!empty($invoice->origin_budget_number))
                <tr><td class="muted small">Presupuesto origen</td><td class="text-right">{{ $invoice->origin_budget_number }}</td></tr>
            @endif
        </table>
    </div>
</section>
...
<section class="mb-3">
    <table class="totals">
        ...
        <tr><td>Impuestos</td><td class="text-right">{{ number_format($invoice->tax_total,2,',','.') }} €</td></tr>
...
<section class="mb-2">
    ...
    <div class="small muted">{{ $invoice->terms }}</div>
</section>
@endif

<footer>
    @php $settings = \App\Models\UserSetting::firstWhere('user_id', $invoice->user_id); @endphp
