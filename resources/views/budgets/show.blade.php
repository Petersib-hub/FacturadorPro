...
<div class="d-flex align-items-center justify-content-between mb-3">
    ...
    <div class="btn-group">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
            ...
            <li>
                <form method="post" action="{{ route('budgets.convert',$budget) }}" onsubmit="return confirm('¿Convertir este presupuesto en factura?')">
                    @csrf
                    <button class="dropdown-item" type="submit">Convertir a factura</button>
                </form>
            </li>
            @php
                $publicUrl = $budget->public_token
                    ? route('public.budgets.show', $budget->public_token)
                    : null;
            @endphp
            @if($publicUrl)
                <li>
                    <a class="dropdown-item" href="#" onclick="copyBudgetPublicLink('{{ $publicUrl }}')">
                        Copiar enlace público
                    </a>
                </li>
                @if($budget->client?->public_token)
                    <li>
                        <a class="dropdown-item" href="{{ route('public.portal.client',$budget->client->public_token) }}" target="_blank" rel="noopener">
            ...
            <li>
                <a class="dropdown-item" href="{{ route('budgets.edit',$budget) }}">Editar</a>
            </li>
        </ul>
    </div>
</div>
            ...
            <div class="modal-body">
                <div class="mb-2">
                    ...
                    <input type="email" name="to" class="form-control" required
                        value="{{ old('to',$budget->client->email ?? '') }}"
