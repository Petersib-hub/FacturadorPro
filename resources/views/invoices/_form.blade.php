...
<div class="row g-3">
    <div class="col-md-6">
        ...
        <select name="client_id" class="form-select" required>
            @foreach(($clients ?? []) as $c)
                <option value="{{ $c->id }}" @selected(old('client_id', $invoice->client_id ?? null) == $c->id)>{{ $c->name }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-3">
    ...
    <div class="col-md-3">
        ...
        <input type="date" name="due_date" class="form-control"
               value="{{ old('due_date', ($invoice->due_date ?? now())->toDateString()) }}">
    </div>

    <div class="col-md-3">
        <label class="form-label">Moneda</label>
        <input name="currency" class="form-control"
               value="{{ old('currency', $invoice->currency ?? ($settings->currency_code ?? 'EUR')) }}">
    </div>
</div>
...
<small class="text-muted d-block mb-2">La descripción es un <em>textarea</em> auto-ajustable. Si es muy larga, aparecerá scroll vertical.</small>
...
<div class="table-responsive">
    <table class="table align-middle" id="itemsTable">
        ...
        <tbody>
        @php
            $oldItems = old('items', isset($invoice) ? $invoice->items->toArray() : [['description'=>'','quantity'=>1,'unit_price'=>0,'discount'=>0,'tax_rate'=>0]]);
        @endphp
...
(function(){
    ...
    function hookInputs(scope){
        scope.querySelectorAll('.qty,.price,.disc,.tax').forEach(i=>{
            i.addEventListener('input', recalc);
        });
        scope.querySelectorAll('textarea.desc').forEach(t=>{
            autosize(t);
